...
class BimanualYamCameraSystem(CameraSystemConfig):
    ...
    cameras: list[AllCameraTypes] = [
        # ===== 对齐官方 MolmoAct2 sim_eval/robots/bimanual_yam.py (top/left/right) =====
        # 官方 left_cam: RealSense D405, 挂 left_link_6, HFOV 87° -> vfovy 56.2° (640x360)
        RobotMountedCameraConfig(
            name="left_wrist_camera",  # policy camera_mapping -> observation.images.left
            reference_body_names=["robot_0/left_link_6"],
            camera_offset=[0.0, 0.09, 0.06],
            camera_quaternion=[0.0, 0.0, 0.965926, 0.258819],
            fov=56.2,
        ),
        # 官方 right_cam: 与 left 同偏移/朝向, 挂 right_link_6
        RobotMountedCameraConfig(
            name="right_wrist_camera",  # policy camera_mapping -> observation.images.right
            reference_body_names=["robot_0/right_link_6"],
            camera_offset=[0.0, 0.09, 0.06],
            camera_quaternion=[0.0, 0.0, 0.965926, 0.258819],
            fov=56.2,
        ),
        # 官方 top_cam: RealSense D435i, 挂 bimanual_base, HFOV 69.4° -> vfovy 42.6°
        RobotMountedCameraConfig(
            name="exo_camera",  # policy camera_mapping -> observation.images.top
            reference_body_names=["robot_0/bimanual_base", "robot_0/base"],
            camera_offset=[0.15, 0.0, 0.8],
            camera_quaternion=[-0.704416, -0.061628, 0.061628, 0.704416],
            fov=42.6,
            visibility_constraints={
                "__task_objects__": 0.001,
            },
