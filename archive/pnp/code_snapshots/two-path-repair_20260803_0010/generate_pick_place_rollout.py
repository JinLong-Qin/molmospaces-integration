...
def _molmospaces_joint_execute(
    ...
    executed_waypoint_poses, executed_waypoint_subtasks = [], []
    ...
    for subtask_index, seq in enumerate(self.waypoint_sequences):
        for j in range(len(seq)):
            ...
            for waypoint_step in range(max_steps):
                ...
                executed_waypoint_poses.append(np.asarray(waypoint.pose, dtype=np.float32))
                executed_waypoint_subtasks.append(subtask_index)
                ...
                if args.stop_on_success and bool(cur_success_metrics.get("task", False)):
                    return dict(
                        ...
                        waypoint_poses=np.asarray(executed_waypoint_poses),
                        waypoint_subtasks=np.asarray(executed_waypoint_subtasks, dtype=np.int32),
...
class MolmoSpacesPnpEnv(EnvBase):
    ...
    def step(self, action):
        ...
        if action.shape[0] == 7 and args.rollout_action_type == "tcp_delta":
            ...
            qdot = np.asarray(
                self.robot.kinematics.twist_to_joint_vel(
                    "arm",
                    twist_world,
                    unlocked,
                    q0,
            ...
            jacobian = self.robot.robot_view.get_jacobian("arm", unlocked)
            damped_inverse = jacobian.T @ np.linalg.solve(
                jacobian @ jacobian.T + args.tcp_ik_damping * np.eye(jacobian.shape[0]),
...
try:
    ...
    waypoint_poses = np.asarray(results["waypoint_poses"], dtype=np.float32)
    waypoint_subtasks = np.asarray(results["waypoint_subtasks"], dtype=np.int32)
