...
exec "$PY" "$ROOT/src/pnp/run_generation.py" \
  --root "$ROOT" --python "$PY" --work "$WORK" --mode per-subtask --diagnostic \
  --source-count "${SOURCE_COUNT_EXPECTED:-19}" --source-hdf5 "${PNP_SOURCE_HDF5:?Set PNP_SOURCE_HDF5}" \
  ...
  --target-end "${TARGET_END:--1}" --rng-seed-base "${RNG_SEED_BASE:-10000}" \
  --run-label "${RUN_LABEL:-quarantined_repair_per_subtask}"
