...
WORK = Path(os.environ.get("MOLMOSPACES_PNP_WORKDIR", ROOT / "runtime/mimicgen_pick_and_place"))
parser = argparse.ArgumentParser()
parser.add_argument("--seed-index", type=int, required=True)
parser.add_argument(
    "--manifest", default=str(WORK / "artifacts/seeds/pnp_seed_manifest_50demo_crossmix.json")
)
parser.add_argument("--out-root", default=str(WORK / "artifacts/replay_pnp_exact_50cross"))
...
raw_dir = seed_meta.get("raw_h5_dir", "raw")
...
log(
    f"selected PNP pool50 seed={args.seed_index:02d} house={HOUSE_ID} {TRAJ}: T={len(actions)} task={obs_scene.get(TASK_KEY)!r} orig_success={orig_success}"
)
...
try:
    ...
    if e.name != "nltk":
        raise
    log("nltk not installed; continuing")
...
(out / "replay_trace.json").write_text(json.dumps(trace, indent=2, ensure_ascii=False))
