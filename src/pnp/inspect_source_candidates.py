                        ...
                        try:
                            with h5py.File(tmp, "r") as h:
                                print(
                                    ...
                                    list(h.keys()),
                                    "valid_mask",
                                    np.asarray(h.get("valid_traj_mask", [])).tolist(),
                                )
                                keys = sorted(
                                ...
                                for k in keys:
                                    ...
                                    try:
                                        ...
                                        obs = {"ERR": repr(e)}
                                    first = (
                                        int(np.argmax(success))
                                        if len(success) and success.any()
                                        else -1
                                    )
                                    pers = (
                                        first >= 0
                                        and bool(success[-1])
                                        and bool(success[first:].all())
                                    )
                                    print(
                                        " TRAJ",
                                        k,
                                        ...
                                        len(success),
                                        "final",
                                        bool(success[-1]) if len(success) else None,
                                        "first",
                                        first,
                                        "pers",
                                        pers,
