...
def main():
    ...
    ap.add_argument(
        "--out", type=Path, default=WORK / "artifacts/seeds/robomimic_pnp_seed00_aligned.hdf5"
    )
    args = ap.parse_args()
    ...
    with h5py.File(args.out, "w") as out:
        ...
        demo.attrs["alignment"] = (
            "real commanded_action rows only: rows 1..153; final success marker row excluded; pre_obs=row-1, post_obs=row"
        )
    ...
    print(
        ...
        n,
        "success_first",
        int(np.argmax(success)) if success.any() else -1,
        "success_final",
        bool(success[-1]),
    )
