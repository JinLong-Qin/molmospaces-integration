...
ap.add_argument('--manifest', default=str(WORK/'artifacts/seeds/pnp_seed_manifest_homogeneous_potato_tomato_bowl.json'))
ap.add_argument('--out-root', default=str(WORK/'artifacts/replay_pnp_exact_homogeneous_potato_tomato_bowl'))
args = ap.parse_args()
T0 = time.monotonic()
...
log('selected HOM PNP seed={:02d} house={} {}: T={} object={} place={} orig_success={}'.format(args.seed_index, HOUSE_ID, TRAJ, len(actions), obs_scene.get('object_name'), obs_scene.get('place_receptacle_name'), orig_success))
...
for action_i, act in enumerate(actions):
    ...
    row = {'action_i': action_i, 'obs_i': obs_i, 'phase': phase, 'success': success, 'position_error': float(info0.get('position_error', -1)), 'supported_by_receptacle': bool(info0.get('supported_by_receptacle', False)), 'robot_contact': bool(info0.get('robot_contact', False)) if 'robot_contact' in info0 else None, 'qerr_same': float(np.max(np.abs(qpos - seed_panda_all[obs_i, :9])))}
    trace.append(row)
...
log(f'VERDICT first_success={first_success} final={final} success_persistent={success_persistent} original={orig_success}')

out = Path(args.out_root) / f'seed_{args.seed_index:02d}'
out.mkdir(parents=True, exist_ok=True)
np.save(out / 'pickup_obj_pose_rel_observations.npy', np.asarray(pickup_pose_rel_observations, dtype=np.float32))
np.save(out / 'place_receptacle_pose_rel_observations.npy', np.asarray(place_pose_rel_observations, dtype=np.float32))
np.save(out / 'target_pose_rel_actions.npy', np.asarray(target_pose_rel_actions, dtype=np.float32))
summary = {'seed': f'seed_{args.seed_index:02d}', 'house_id': HOUSE_ID, 'traj': TRAJ, 'task_description': obs_scene.get('task_description'), 'pickup_obj_name': pickup, 'place_receptacle_name': place, 'actions_replayed': len(actions), 'first_success_obs': first_success, 'final_success': final, 'original_success': orig_success, 'success_persistent_to_end': success_persistent, 'initial_gate': {'obj_maxerr': obj_err, 'base_maxerr': base_err, 'arm_maxerr': arm_err, 'gripper_maxerr': grip_err}, 'trajectory_fidelity': {'max_joint_error_rad': max(r['qerr_same'] for r in trace), 'final_joint_error_rad': trace[-1]['qerr_same']}, 'saved_arrays': {'pickup_obj_pose_rel_observations': list(np.asarray(pickup_pose_rel_observations).shape), 'place_receptacle_pose_rel_observations': list(np.asarray(place_pose_rel_observations).shape), 'target_pose_rel_actions': list(np.asarray(target_pose_rel_actions).shape)}}
(out / 'datagen_info_collection_result.json').write_text(json.dumps(summary, indent=2, ensure_ascii=False))
