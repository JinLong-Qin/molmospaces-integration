...
wanted = [
    ...
    (1536, 1, 'replacement: tomato -> terracotta bowl; 2423/traj1 excluded because valid_traj_mask=False'),
]
by_house={}
for h,t,d in wanted: by_house.setdefault(h,[]).append((t,d))
    ...
    try:
        with h5py.File(tmp,'r') as h:
            ...
            for k in keys:
                ...
                rows.append({
                    'traj_index':ti, 'traj_key':k, 'valid_traj_mask':valid_mask,
                    'length':int(len(success)), 'success_final':bool(success[-1]) if len(success) else False,
                    'first_success':first, 'success_persistent_to_end_metadata':pers,
                    'object_name':obs.get('object_name'), 'pickup_obj_name':obs.get('pickup_obj_name'),
                    'place_receptacle_name':obs.get('place_receptacle_name'), 'task_type':obs.get('task_type'),
                    'task_description':obs.get('task_description'), 'house_id':house_id, 'batch_id':batch_id,
                    'source_outer_member':outer_name, 'source_inner_h5':inner_h5,
                })
...
with tarfile.open(TAR,'r') as outer:
    for house_id, wants in by_house.items():
        ...
        try:
            h5_bytes=None; inner_h5=None; batch_id=-1
            with tarfile.open(fileobj=reader, mode='r|') as inner:
                for hm in inner:
                    if hm.isfile() and hm.name.endswith('.h5'):
                        h5_bytes=inner.extractfile(hm).read(); inner_h5=hm.name
                        m=re.search(r'trajectories_batch_(\d+)_', hm.name); batch_id=int(m.group(1)) if m else -1
                        break
            ...
            for traj_index, note in wants:
                ...
                if not (r['valid_traj_mask'] and r['success_final'] and r['success_persistent_to_end_metadata']):
                    raise RuntimeError('candidate failed official pregate '+json.dumps(r, ensure_ascii=False))
                raw_name=f'hom_seed_{len(found):02d}__house{house_id}__batch{batch_id}.h5'
                (RAW/raw_name).write_bytes(h5_bytes)
                r.update({'seed_index':len(found), 'candidate_note':note, 'raw_h5':raw_name, 'raw_h5_dir':str(RAW.relative_to(OUT)), 'seed_kind':'synthetic_planner_expert', 'selection_family':'potato_tomato_to_bowl'})
                found.append(r)
        finally:
            try: reader.close()
            except Exception: pass
manifest={'dataset':'FrankaPickAndPlaceOmniCamConfig','split':'val_shards/00000.tar','selection_note':'homogeneous potato/tomato -> bowl source set; 2423/traj1 visual candidate excluded due valid_traj_mask=False and replaced by 1536/traj1','seed_kind':'synthetic_planner_expert (cuRobo/scripted), NOT human demonstrations','excluded_visual_candidates':[{'house_id':2423,'traj_index':1,'reason':'valid_traj_mask=False'}],'seeds':found}
out=OUT/'pnp_seed_manifest_homogeneous_potato_tomato_bowl.json'
out.write_text(json.dumps(manifest, indent=2, ensure_ascii=False))
print(json.dumps({'out':str(out),'n':len(found),'seeds':[{k:s[k] for k in ['seed_index','house_id','traj_key','valid_traj_mask','task_description','first_success','length','raw_h5']} for s in found]},indent=2,ensure_ascii=False))
