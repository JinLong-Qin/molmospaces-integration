...
# Current JsonEvalTaskSampler asserts benchmark PnP receptacle displacement tolerance is 0.15.
...
try:
    ...
    raise RuntimeError(f"pickup body absent: {pickup!r}")
try:
    rid = model.body(place).id
except KeyError:
    log(f"warning: place receptacle body lookup failed: {place!r}")
    rid = None
...
log(f"replaying {len(actions)} commanded actions")
first_success = -1
