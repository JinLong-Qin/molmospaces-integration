...
def build_demo(seed_index: int, demo_name: str, out_data, manifest):
    seed = manifest['seeds'][seed_index]
    raw_h5 = WORK/'artifacts/seeds/raw'/seed['raw_h5']
    traj = seed['traj_key']
    replay_dir = WORK/f'artifacts/replay_pnp_exact/seed_{seed_index:02d}'
    replay_result = json.loads((replay_dir/'replay_result.json').read_text())
    ...
    with h5py.File(raw_h5, 'r') as f:
        ...
        if not real_action_rows or real_action_rows[0] != 1:
            raise RuntimeError(f'seed_{seed_index:02d}: unexpected real action rows start {real_action_rows[:3]}')
    ...
    if success.any(): dones[int(np.argmax(success)):] = 1
    else: dones[-1] = 1
    ...
    for k,v in signals.items(): sig.create_dataset(k, data=v)
    ...
    demo.attrs['alignment'] = 'real commanded_action rows only; final success marker row excluded; pre_obs=row-1, post_obs=row'
    ...
    return {'demo': demo_name, 'seed_index': seed_index, 'house_id': int(seed['house_id']), 'n': n, 'signals': signal_indices, 'first_success': int(np.argmax(success)) if success.any() else -1, 'success_final': bool(success[-1]), 'task': obs_scene.get('task_description','')}

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--accepted', default=','.join(map(str, ACCEPTED_DEFAULT)))
    ap.add_argument('--out', type=Path, default=WORK/'artifacts/seeds/robomimic_pnp_10demo_aligned.hdf5')
    args = ap.parse_args()
    accepted = [int(x) for x in args.accepted.split(',') if x.strip()]
    ...
    with h5py.File(args.out, 'w') as out:
        ...
        data.attrs['env_args'] = json.dumps({'env_name':'MolmoSpacesPickAndPlaceEnv','env_kwargs':{'config':'FrankaPickAndPlaceOmniCamConfig','source_demo_count':len(accepted)},'type':2,'note':'MolmoSpaces PnP synthetic planner expert source demos for MimicGen integration'})
        for demo_i, seed_index in enumerate(accepted):
            s = build_demo(seed_index, f'demo_{demo_i}', data, manifest)
            summaries.append(s); total += s['n']
    ...
    summary_path.write_text(json.dumps({'out':str(args.out),'accepted':accepted,'n_demos':len(accepted),'total':total,'demos':summaries}, indent=2, ensure_ascii=False))
