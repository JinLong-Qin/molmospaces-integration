...
CHUNK_SIZE = 30
OUT = Path(
    "artifacts/molmospaces/molmoact2_yam_integration/official_sim_eval_bridge_act30_stream_report.json"
)
...
def main():
    ...
    cfg.policy_config.num_steps = 30
    cfg.policy_config.n_action_steps = None
    cfg.policy_config.timeout = 120.0
    cfg.policy_config.raw_action_log_path = "artifacts/molmospaces/molmoact2_yam_integration/official_sim_eval_bridge_act30_stream_raw_actions.jsonl"
    cfg.policy_config.debug_dump_dir = "artifacts/molmospaces/molmoact2_yam_integration/official_sim_eval_bridge_act30_stream_debug"
    cfg.policy_config.debug_dump_max_calls = 2

    report = {
        "execution_mode": cfg.policy_config.execution_mode,
        "evidence_level": "bounded official-protocol /act diagnostic: one 30-action chunk buffered and executed sequentially; not a long rollout",
        "endpoint_url": None,
        ...
        while report["actions_executed"] < MAX_ACTIONS:
            ...
            for chunk_idx in range(CHUNK_SIZE):
                ...
                if (
                    ...
                    break
            if report["any_success"] or report["actions_executed"] >= MAX_ACTIONS:
                break
